...
const USHISTORY_COLORS = {
  sage: "#A8434B", sageDeep: "#7A2E34", sagePale: "#F5E6E7", sagePill: "#EBC9CB",
};

const MCQ_STORAGE_KEY = "ushistory-hub-mcq-progress-v1";
const NAV_STORAGE_KEY = "ushistory-hub-last-nav-v1";
...
export default function UsHistoryPage() {
  ...
  const unitFrqs = FRQ_CONTENT[unitId] || {};
  const [frqType, setFrqType] = useState("saq");
  const currentFrq = unitFrqs[frqType] || null;
  ...
  return (
    <div style={S.page}>
      <style>{getEngineCSS(USHISTORY_COLORS)}</style>
      {/* TOP BAR */}
      <div style={S.topbar}>
          ...
          <div style={S.logoRow}>
            <div style={S.logoMark}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
                <path d="M6 3V21" stroke="#7A2E34" strokeWidth="1.8" strokeLinecap="round"/>
                <path d="M6 4H18L15.5 7.5L18 11H6" stroke="#7A2E34" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" fill="none"/>
                <path d="M6 5.5H12.5M6 7.5H12.5M6 9.5H12.5" stroke="#7A2E34" strokeWidth="1" strokeLinecap="round"/>
              </svg>
            </div>
            <div>
              <div style={S.logoText}>AP US History Hub</div>
      ...
      <div style={S.body} className="app-body">
        {mode !== "dashboard" && !globalReview && (
          <button style={S.unitSwitcher} className="mobile-only-switcher" onClick={() => setSidebarOpen(true)}>
            ...
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
              <path d="M4 6L12 14L20 6" stroke="#7A2E34" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </button>
        )}
        ...
        <main style={S.main}>
          ...
          {mode === "frq" && (
            <div style={S.frqWrap}>
              {!currentFrq && Object.keys(unitFrqs).length === 0 ? (
                <div style={S.emptyState}>
                ...
                <>
                  <div style={S.frqTypeRow}>
                    {[
                      { key: "saq", label: "Short Answer" },
                      { key: "dbq", label: "Document-Based Essay" },
                      { key: "leq", label: "Long Essay" },
                    ].map((t) => (
                      <button
                        key={t.key}
                        style={{ ...S.frqTypeBtn, ...(frqType === t.key ? S.frqTypeBtnActive : {}) }}
                        onClick={() => setFrqType(t.key)}
                      >
                        {t.label}
                      </button>
                    ))}
                ...
                <div style={S.frqList}>
                  {currentFrq && currentFrq.parts && (
                    <div style={S.frqCard}>
                      <div style={S.frqHeader}>
                        <div>
                          <div style={S.frqSource}>{currentFrq.type}</div>
                          <div style={S.frqTitle}>{currentFrq.source}</div>
                        </div>
                      </div>
                      <div style={S.frqBody}>
                        <div style={S.frqPrompt} dangerouslySetInnerHTML={{ __html: currentFrq.prompt }} />
                        {currentFrq.parts.map((part, pIdx) => {
                          const key = `${currentFrq.id}::${pIdx}`;
                          const revealed = frqSteps[key];
                          return (
                            <div key={pIdx} style={S.frqPart}>
                              <div style={S.frqPartLabel}>{part.label} {part.points ? `(${part.points} pt)` : ""}</div>
                              <p style={S.frqPartAsk}>{part.ask}</p>
                              {!revealed ? (
                                <button style={S.frqRevealBtn} onClick={() => setFrqSteps({ ...frqSteps, [key]: true })}>
                                  Reveal Model Answer
                                </button>
                              ) : (
                                <div style={S.frqSteps}>
                                  <div style={S.frqStep}>
                                    <span style={S.frqStepText}>{part.guidance}</span>
                                  </div>
                                  <button style={S.frqCollapseBtn} onClick={() => setFrqSteps({ ...frqSteps, [key]: false })}>
                                    Hide Response
                                  </button>
                                </div>
                              )}
                  ...
                  {currentFrq && currentFrq.documents && (
                    <div style={S.frqCard}>
                      <div style={S.frqHeader}>
                        <div>
                          <div style={S.frqSource}>{currentFrq.type}</div>
                      ...
                      <div style={S.frqBody}>
                        {currentFrq.note && <p style={S.frqNote}>{currentFrq.note}</p>}
                        <p style={S.frqPrompt}>{currentFrq.prompt}</p>
                        <div style={S.docList}>
                          {currentFrq.documents.map((doc) => (
                            <div key={doc.num} style={S.docCard}>
                              <div style={S.docLabel}>{doc.label}</div>
                              <p style={S.docDesc}>{doc.desc}</p>
                            </div>
                          ))}
                        </div>
                        {!frqSteps[currentFrq.id] ? (
                          <button style={S.frqRevealBtn} onClick={() => setFrqSteps({ ...frqSteps, [currentFrq.id]: true })}>
                            Reveal Model Outline
                          </button>
                        ) : (
                          <div style={S.outlineBox}>
                            <div style={S.outlineLabel}>Model Thesis</div>
                            <p style={S.outlineThesis}>{currentFrq.modelOutline.thesis}</p>
                            <div style={S.outlineLabel}>Body Paragraph Points</div>
                            {currentFrq.modelOutline.bodyPoints.map((pt, i) => (
                              <div key={i} style={S.frqStep}>
                                <span style={S.frqStepNum}>{i + 1}</span>
                                <span style={S.frqStepText}>{pt}</span>
                              </div>
                            ))}
                            <button style={S.frqCollapseBtn} onClick={() => setFrqSteps({ ...frqSteps, [currentFrq.id]: false })}>
                              Hide Outline
                            </button>
                          </div>
                        )}
                  ...
                  {currentFrq && !currentFrq.parts && !currentFrq.documents && currentFrq.modelOutline && (
                    <div style={S.frqCard}>
                      <div style={S.frqHeader}>
                        <div>
                          <div style={S.frqSource}>{currentFrq.type}</div>
                      ...
                      <div style={S.frqBody}>
                        ...
                        {!frqSteps[currentFrq.id] ? (
                          ...
                          <div style={S.outlineBox}>
                            ...
                            {currentFrq.modelOutline.reasoningSkill && (
                              <>
                                <div style={S.outlineLabel}>Reasoning Skill</div>
                                <p style={S.frqStepText}>{currentFrq.modelOutline.reasoningSkill}</p>
                              </>
                            )}
                            <button style={S.frqCollapseBtn} onClick={() => setFrqSteps({ ...frqSteps, [currentFrq.id]: false })}>
                              Hide Outline
...
const S = {
  ...
  logoMark: { width: 36, height: 36, borderRadius: 11, background: "#F5E6E7", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 },
  ...
  modePillActive: { background: "#A8434B", color: "white" },
  ...
  unitSwitcherNum: { fontFamily: "'IBM Plex Mono',monospace", fontSize: 11.5, color: "#7A2E34", fontWeight: 700 },
  ...
  unitBtnActive: { background: "#F5E6E7", border: "1px solid #B8D1AE" },
  unitBtnTop: { display: "flex", justifyContent: "space-between", marginBottom: 3 },
  unitNum: { fontFamily: "'IBM Plex Mono',monospace", fontSize: 10.5, color: "#7A2E34", fontWeight: 700 },
  ...
  miniFill: { height: "100%", background: "#A8434B" },
  ...
  progressText: { fontFamily: "'Nunito',sans-serif", fontWeight: 700, fontSize: 13.5, color: "#7A2E34" },
  progressTrack: { height: 7, background: "#E6E9E3", borderRadius: 4, overflow: "hidden" },
  progressFill: { height: "100%", background: "#A8434B", transition: "width 0.3s ease" },
  ...
  qNavBtn: { fontFamily: "'Nunito',sans-serif", fontWeight: 700, fontSize: 13.5, background: "#FFFFFF", color: "#7A2E34", border: "1px solid #ECEAE3", padding: "10px 18px", borderRadius: 100, cursor: "pointer", boxShadow: "0 2px 8px rgba(70,90,60,0.05)" },
  ...
  unitBadge: { fontFamily: "'IBM Plex Mono',monospace", fontSize: 10.5, fontWeight: 700, color: "#7A2E34", background: "#EBC9CB", borderRadius: 8, padding: "4px 8px", whiteSpace: "nowrap" },
  ...
  reviewAllCardClear: { background: "#F5E6E7" },
  reviewAllIconClear: { color: "#7A2E34" },
  ...
  filterPillActive: { background: "#A8434B", color: "white" },
  ...
  qNumBadge: { fontFamily: "'IBM Plex Mono',monospace", fontSize: 11.5, fontWeight: 700, color: "#7A2E34", background: "#EBC9CB", borderRadius: 8, padding: "4px 8px" },
  ...
  choiceCorrect: { background: "#E1EEDD", border: "1px solid #A8434B" },
  choiceWrong: { background: "#FBEAE5", border: "1px solid #D98B7B" },
  choiceDisabled: { opacity: 0.5, cursor: "default" },

  explanation: { marginTop: 14, padding: "13px 16px", background: "#F5E6E7", borderRadius: 12 },
  explanationLabel: { fontFamily: "'Nunito',sans-serif", fontWeight: 700, fontSize: 11, letterSpacing: "0.06em", textTransform: "uppercase", color: "#7A2E34", marginBottom: 6 },
  ...
  frqTypeRow: { display: "flex", gap: 8, marginBottom: 16 },
  frqTypeBtn: { fontFamily: "'Nunito',sans-serif", fontWeight: 700, fontSize: 13.5, background: "#FFFFFF", color: "#767F73", border: "1px solid #ECEAE3", padding: "9px 18px", borderRadius: 100, cursor: "pointer" },
  frqTypeBtnActive: { background: "#A8434B", color: "#FFFFFF", border: "1px solid #A8434B" },
  frqNote: { fontSize: 13, color: "#9AA096", fontStyle: "italic", marginBottom: 14, padding: "10px 14px", background: "#F7F6F2", borderRadius: 10 },
  docList: { display: "flex", flexDirection: "column", gap: 10, margin: "16px 0" },
  docCard: { background: "#F7F6F2", border: "1px solid #ECEAE3", borderRadius: 12, padding: "12px 16px" },
  docLabel: { fontFamily: "'IBM Plex Mono',monospace", fontWeight: 700, fontSize: 12.5, color: "#A8434B", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.04em" },
  docDesc: { fontSize: 14, lineHeight: 1.6, color: "#2E332E" },
  outlineBox: { marginTop: 14, padding: "16px 18px", background: "#F5E6E7", borderRadius: 14 },
  outlineLabel: { fontFamily: "'IBM Plex Mono',monospace", fontWeight: 700, fontSize: 12, color: "#7A2E34", textTransform: "uppercase", letterSpacing: "0.04em", marginTop: 12, marginBottom: 8 },
  outlineThesis: { fontSize: 14.5, lineHeight: 1.7, color: "#2E332E", fontStyle: "italic" },
  ...
  frqPartLabel: { fontFamily: "'IBM Plex Mono',monospace", fontWeight: 700, fontSize: 12.5, color: "#7A2E34", background: "#F5E6E7", display: "inline-block", padding: "3px 10px", borderRadius: 100, marginBottom: 8 },
  ...
  emptyIconBadge: { width: 56, height: 56, borderRadius: 18, background: "#F5E6E7", color: "#7A2E34", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" },
  ...
  dashCardValue: { fontFamily: "'Manrope',sans-serif", fontWeight: 800, fontSize: 28, color: "#7A2E34" },
  ...
  dashUnitFill: { height: "100%", background: "#A8434B" },
  dashUnitActions: { display: "flex", gap: 8, marginTop: "auto" },
  dashJumpBtn: { fontFamily: "'Nunito',sans-serif", fontWeight: 700, fontSize: 12, background: "#F5E6E7", color: "#7A2E34", border: "none", borderRadius: 100, padding: "6px 14px", cursor: "pointer" },
