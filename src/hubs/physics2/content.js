...
const QUESTIONS = {
  9: [
    ...
    { id: "9-8", topic: "9.4 First Law of Thermodynamics", stem: "A rigid, sealed container of gas is heated by an external heater that adds energy to it; its pressure rises but no piston motion occurs. What can be said about the work done on the gas and its internal energy?", choices: ["W = 0, and ΔU = Q, so internal energy increases.", "W = Q, and ΔU = 0.", "W is negative, since the gas is being compressed.", "ΔU = 0, since volume did not change."], correct: 0, explanation: "This question is testing whether you conflate 'compression' with 'work' correctly — work done on a gas via pressure-volume changes requires an actual volume change (W = −PΔV for a constant-pressure case, or more generally the area under a P-V curve). Here, volume is explicitly constant (isochoric), so no P-V work is done regardless of what's happening to pressure: W = 0. All the added heat goes directly into internal energy: ΔU = Q. This is why isochoric processes are the simplest case of the first law — one term drops out entirely." },
    ...
    { id: "9-11", topic: "9.6 Entropy", stem: "A cup of hot coffee is left on a table and slowly cools to room temperature. Which statement correctly applies the Second Law of Thermodynamics to this process?", choices: ["The entropy of the coffee decreases as it cools, and this is consistent with the second law because entropy is only ever defined for isolated systems.", "The total entropy of the coffee-plus-room system increases, since heat is flowing spontaneously from hot to cold.", "The entropy of the room decreases as it absorbs heat, violating the second law.", "Entropy is not applicable to this process since no phase change occurs."], correct: 1, explanation: "Heat flowing spontaneously from a hotter object (the coffee) to a colder surroundings (the room) is exactly the kind of process the second law describes as entropy-increasing overall. Even though the coffee itself loses some entropy as it cools, the room gains more than enough entropy to make the total (coffee + room) increase — that's why the process happens spontaneously in this direction and never spontaneously in reverse." },
    { id: "9-12", topic: "9.2 Ideal Gas Law", stem: "Two balloons (flexible, able to expand or contract to maintain a fixed internal pressure) hold ideal gas at the same temperature and pressure. Balloon A holds twice as many moles of gas as Balloon B. How do their volumes compare?", choices: ["Balloon A has twice the volume of Balloon B.", "Balloon A has half the volume of Balloon B.", "The balloons have the same volume, since they're at the same temperature and pressure.", "The relationship cannot be determined without knowing the molar mass."], correct: 0, explanation: "Using PV = nRT with P and T fixed, V is proportional to n. Twice the moles means twice the volume of gas, since the flexible balloon can expand to accommodate the extra gas while keeping pressure and temperature constant. The key skill is applying V ∝ n directly from the ideal gas law. (Note: this only works because the containers are flexible — if they were identical *rigid* containers, fixing V for both, then equal P and T would force equal n instead, making 'twice the moles' impossible.)" },
  ...
  14: [
    ...
    { id: "14-28", topic: "14.5 Doppler Effect", stretch: true, stem: "Two cars move directly toward each other, one honking its horn continuously. Compare the observed frequency in this case to a scenario where only the honking car moves (listener stationary) at the same total relative closing speed.", choices: ["The two-cars-moving scenario produces a LARGER frequency shift than the single-mover scenario at the same relative speed.", "The two-cars-moving scenario produces a SMALLER frequency shift.", "The frequency shift is identical in both cases, since only relative speed matters.", "No Doppler shift occurs unless the source alone is moving."], correct: 1, explanation: "The two-cars-moving case produces a smaller shift. In the full Doppler formula f' = f(v + vo)/(v − vs), source motion toward the observer affects the denominator (v − vs) while observer motion toward the source affects the numerator (v + vo). These two terms aren't equally sensitive: the denominator's effect grows nonlinearly as vs increases, while the numerator's effect is only linear in vo. So concentrating the entire relative closing speed into source motion alone (single-mover case) produces a larger shift than splitting the same total speed between source and observer. For example, with v = 340 m/s and a 30 m/s total closing speed: putting it all on the source gives f' = f(340/310) ≈ 1.097f, while splitting it evenly (15 m/s each) gives f' = f(355/325) ≈ 1.092f — smaller. This requires reasoning about combined source-and-observer motion together, rather than the simpler single-mover scenarios more commonly emphasized." },
